...
initialProducts.forEach((data) => {
    repository.add(new Product(data.id, data.name, data.price, data.category));
});
repository.getAll().forEach((product) => {
    console.log(`${product.name}: ${formatEuro(product.price, config.currency)}`);
});
const monitor = repository.findById(2);
if (monitor !== undefined) {
    monitor.applyDiscount(10);
    console.log(`Discounted monitor: ${formatEuro(monitor.price, config.currency)}`);
}
