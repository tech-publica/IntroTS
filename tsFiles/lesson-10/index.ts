...
const personPromise: Promise<SwapiPerson> = getSwapiPerson(1);
...
let peopleState: RequestState<SwapiPerson[]> = {
  status: "idle",
};

async function runLesson(): Promise<void> {
  ...
  try {
    ...
    people.forEach((person) => {
      console.log(
        `${person.name}: ${person.height} cm, ${person.films.length} films`,
      );
    });

    const createdPost: JsonPlaceholderPost =
      await createJsonPlaceholderPost({
        userId: 1,
...
console.log(
  "Cancellation signal initially aborted:",
  unusedControllerExample.signal.aborted,
);
